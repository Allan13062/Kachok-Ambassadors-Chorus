...
export default function AdminPanel({
  ...
  const fetchDonations = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setDonationsList(await res.json());
      }
    } catch (err) {
  ...
  // Uploads directly to Cloudinary (bypassing Vercel's 4.5MB function body
  // limit) and returns the final URL. Throws on failure — callers must catch
  // this and tell the admin, rather than silently pretending it saved.
  const uploadBase64 = async (base64Str: string | undefined | null, defaultFilename = "upload.jpg"): Promise<string | undefined | null> => {
    if (!base64Str || !base64Str.startsWith("data:")) {
      return base64Str;
    }
    return uploadMedia(base64Str, adminToken || "", defaultFilename);
  };
        ...
        {authLoading && !isAuthenticated ? (
          ...
          <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-6 sm:px-8 py-6 flex flex-col gap-10 custom-scrollbar max-h-[70vh]">
              ...
              {/* SECTION: RECENT M-PESA CONTRIBUTIONS (real, persisted transaction records) */}
              <div id="admin-donations-section" className="bg-slate-950/40 border border-slate-800 p-6 rounded-2xl">
                <div className="flex items-center justify-between gap-2 mb-4">
                  <div className="flex items-center gap-2 text-amber-400 bg-slate-950 p-2.5 rounded-lg border border-slate-805 text-sm font-bold uppercase tracking-wider">
                    <DollarSign className="w-5 h-5 text-amber-500" />
                    <span>Recent Contributions</span>
                  </div>
                  <button
                    onClick={fetchDonations}
                    className="text-xs font-mono text-slate-400 hover:text-amber-400 uppercase tracking-wide cursor-pointer"
                  >
                ...
                {donationsList.length === 0 ? (
                  <p className="text-xs text-slate-500 font-sans py-6 text-center">
                    {donationsLoading ? "Loading contributions..." : "No contributions recorded yet."}
                  </p>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs font-sans">
                      <thead>
                        <tr className="text-slate-500 uppercase text-[10px] font-mono tracking-wide border-b border-slate-800">
                          <th className="text-left py-2 pr-4">Phone</th>
                          <th className="text-left py-2 pr-4">Amount</th>
                          <th className="text-left py-2 pr-4">Status</th>
                          <th className="text-left py-2 pr-4">Receipt No.</th>
                      ...
                      <tbody>
                        {donationsList.map((d) => (
                          <tr key={d.id} className="border-b border-slate-900 text-slate-300">
                            <td className="py-2 pr-4 font-mono">{d.phone}</td>
                            <td className="py-2 pr-4 font-mono">KES {d.amount}</td>
                            <td className="py-2 pr-4">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${
                                ...
                                {d.status}{d.simulated === "true" ? " (sim)" : ""}
                              </span>
                            </td>
                            <td className="py-2 pr-4 font-mono">{d.mpesaReceiptNumber || "—"}</td>
                            <td className="py-2 pr-4 text-slate-500">
                              {d.createdAt ? new Date(d.createdAt).toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" }) : "—"}
                            </td>
                          </tr>
                        ))}
                      </tbody>
