...
export default function JoinUs() {
  ...
  const [pushStatus, setPushStatus] = useState<{
    type: "idle" | "loading" | "awaiting" | "success" | "failed" | "timeout" | "error";
    message: string;
    refNumber?: string;
  }>({ type: "idle", message: "" });
  ...
  const secondsRef = React.useRef<ReturnType<typeof setInterval> | null>(null);

  // Always clear any in-flight polling if the component unmounts (e.g. user navigates away
  // mid-payment) so we don't keep hitting the server for a screen no one is looking at.
  useEffect(() => {
    return () => {
  ...
  // Polls our own /api/mpesa/status endpoint — which reflects Safaricom's real callback —
  // rather than ever assuming success just because the prompt was delivered.
  const pollForConfirmation = (checkoutRequestId: string, finalAmount: string) => {
    setAwaitingSeconds(0);
    secondsRef.current = setInterval(() => setAwaitingSeconds(s => s + 1), 1000);

    const maxAttempts = 30; // ~90 seconds at 3s intervals — STK prompts expire around then anyway
    let attempts = 0;
    ...
    pollRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.status === "completed") {
            ...
            setPushStatus({
              ...
              refNumber: data.mpesaReceiptNumber || checkoutRequestId
            });
            setIsPushing(false);
            return;
          }
          if (data.status === "failed") {
            ...
            setPushStatus({
              type: "failed",
              message: data.resultDesc || "The payment wasn't completed. You may have cancelled the prompt or entered the wrong PIN — no money left your account."
            });
            setIsPushing(false);
            return;
          }
        ...
        // A transient network hiccup while polling isn't itself a payment failure — just keep trying.
      }
      ...
      if (attempts >= maxAttempts) {
        ...
        setPushStatus({
          type: "timeout",
          message: "We haven't heard back from M-Pesa yet."
        });
        setIsPushing(false);
      }
    }, 3000);
  ...
  const handleStkPushSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        if (data.realApi) {
          // Real Safaricom push accepted — this only means the prompt reached the phone,
          // NOT that payment happened. Wait for real confirmation before saying anything succeeded.
          setPushStatus({
            type: "awaiting",
            message: data.message,
            refNumber: data.checkoutRequestId
          });
          pollForConfirmation(data.checkoutRequestId, finalAmount);
        } else {
          // No live API keys configured — this is a clearly-labeled local/dev simulation only.
          setPushStatus({
            type: "loading",
            message: data.message,
          ...
          setTimeout(() => {
            setPushStatus({
              type: "success",
              message: "✓ Simulated contribution of KES " + finalAmount + " processed successfully! (No real payment was made — configure M-Pesa API keys to accept live contributions.)",
              refNumber: data.checkoutRequestId || `ws_CO_${Date.now().toString().slice(3)}`
            });
            setIsPushing(false);
          }, 3000);
                  ...
                  {pushStatus.type === "success" ? (
                              ...
                              content = (
                                <div className="bg-slate-50 rounded-xl p-4 mb-8 border border-slate-100 text-sm">
                                  <div className="flex justify-between items-center py-2 border-b border-slate-200 border-dashed">
                                    <span className="text-slate-500">M-Pesa Receipt No.</span>
                    ...
                    <div className="flex flex-col items-center justify-center text-center py-10 px-4">
                      <div className="relative w-24 h-24 mb-6 flex items-center justify-center">
                        ...
                        />
                        <div className="relative w-14 h-14 bg-amber-500 rounded-full flex items-center justify-center shadow-lg shadow-amber-500/30">
                          <Smartphone className="w-7 h-7 text-slate-950" />
                        </div>
                      </div>

                      <h3 className="font-sans font-bold text-xl text-white mb-1.5">Check your phone</h3>
                      <p className="font-sans text-sm text-slate-400 max-w-xs leading-relaxed">
                        Enter your M-Pesa PIN to complete your gift of <span className="text-amber-400 font-semibold">KES {amount === "custom" ? customAmount : amount}</span>.
                      </p>
                      ...
                      <div className="w-full max-w-[220px] mt-7">
                        <div className="h-1 bg-slate-800 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: `${Math.min((awaitingSeconds / 90) * 100, 100)}%` }}
                          />
                        </div>
                        <p className="font-mono text-[11px] text-slate-500 mt-2 tabular-nums">
                          Waiting for confirmation · {Math.floor(awaitingSeconds / 60)}:{String(awaitingSeconds % 60).padStart(2, "0")}
                        </p>
                      </div>
                      ...
                      <button
                        ...
                        className="mt-8 text-xs text-slate-500 hover:text-slate-300 font-medium underline cursor-pointer"
                      >
                        Cancel and start over
                      </button>
                    </div>
                  ) : pushStatus.type === "failed" ? (
                    <div className="flex flex-col items-center justify-center text-center py-10 px-4">
                      <div className="w-16 h-16 bg-red-500/10 rounded-full flex items-center justify-center mb-5">
                        <X className="w-8 h-8 text-red-400" />
                      </div>
                      <h3 className="font-sans font-bold text-xl text-white mb-1.5">Payment not completed</h3>
                      <p className="font-sans text-sm text-slate-400 max-w-xs leading-relaxed">
                        {pushStatus.message}
                      </p>
                      <p className="font-sans text-xs text-slate-500 mt-3">No money was deducted from your account.</p>
                      <button
                        onClick={() => setPushStatus({ type: "idle", message: "" })}
                        className="mt-7 bg-amber-500 hover:bg-amber-400 text-slate-950 font-sans font-bold py-2.5 px-6 rounded-xl transition-all cursor-pointer text-sm"
                      >
                        Try Again
                      </button>
                    </div>
                  ) : pushStatus.type === "timeout" ? (
                    <div className="flex flex-col items-center justify-center text-center py-10 px-4">
                      ...
                      <h3 className="font-sans font-bold text-xl text-white mb-1.5">Still waiting to hear back</h3>
                      <p className="font-sans text-sm text-slate-400 max-w-xs leading-relaxed">
                        We haven't received confirmation from M-Pesa yet. If you already entered your PIN, your contribution may still go through — check your M-Pesa messages to confirm.
                      </p>
                      <div className="flex gap-3 mt-7">
                        <button
                          onClick={() => setPushStatus({ type: "idle", message: "" })}
                          className="bg-slate-800 hover:bg-slate-700 text-white font-sans font-semibold py-2.5 px-5 rounded-xl transition-all cursor-pointer text-sm"
                        >
                          Try Again
                        </button>
                      </div>
